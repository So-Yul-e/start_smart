...
**현재 상태**:
- TODO 주석으로 Mock 데이터 사용 중
- 다른 모듈들이 병합되면 주석 해제 필요

**해야 할 일**:
- [ ] `engine/finance` 모듈 병합 후 `calculateFinance` import
- [ ] `ai/roadview` 모듈 병합 후 `analyzeRoadview` import
- [ ] `ai/consulting` 모듈 병합 후 `generateConsulting` import
- [ ] `engine/decision` 모듈 병합 후 `calculateDecision` import
