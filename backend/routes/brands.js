...
async function getAllBrands() {
  try {
    ...
    console.warn('[getAllBrands] DB 조회 실패, fallback 데이터 사용:', error.message);
  }
  
  // DB 실패 시 fallback 데이터 반환
...
async function getBrandById(brandId) {
  try {
    ...
    console.warn('[getBrandById] DB 조회 실패, fallback 데이터 사용:', error.message);
  }
  
  // DB 실패 시 fallback 데이터에서 찾기
  const fallback = fallbackBrands.find(b => b.id === brandId);
  if (fallback) {
    console.log('[getBrandById] fallback 데이터 사용:', brandId);
    return fallback;
  }
